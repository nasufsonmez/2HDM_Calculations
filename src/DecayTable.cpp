...
double DecayTable::get_gamma_hdd(int h, int d1, int d2) {
  ...
  model.get_coupling_hdd(h,d1,d2,cs,cp);
    ...
    //NASUF
   //cout << "couplings (" << h << d1 << d2 <<  ") cs cp :   "  << cs << "    "  << cp << endl;
...
double DecayTable::get_gamma_hhh(int h, int h1, int h2)
{
  ...
  if (M>(m1+m2)) {
    double M2    = pow(abs(c),2);
      //NASUF
      //cout << " matrix element : " << c << "   Sf : " << Sf << endl;
...
double DecayTable::get_gammatot_h(int h) 
{
  ...
  for (int i=1;i<4;i++) {
    for (int j=1;j<4;j++) {
        //  if (get_gamma_huu(h,i,j)) cout << "get_gamma_huu(" << h << i << j << ")   :    " << get_gamma_huu(h,i,j) << endl;
        //	if (get_gamma_huu(h,i,j)) cout << "get_gamma_hdd(" << h << i << j << ")   :    	" << get_gamma_hdd(h,i,j) << endl;
  ...
  for (int i=1;i<5;i++) {
    for (int j=1;j<5;j++) {
       ...
       // cout << "get_gamma_hhh("<<h<<","<<i<<","<<j<<")  : " << get_gamma_hhh(h,i,j)<< endl;
    }
  }
