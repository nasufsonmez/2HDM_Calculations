...
bool THDM::set_param_hybrid(
	double mh, double mH, double cba, double Z4, double Z5, double Z7, double tanb) {
...
bool THDM::set_param_hybrid_sba(
double mh, double mH, 
double sba, 
double Z4,
double Z5, double Z7, 
...
void THDM::get_param_phys(double &m_h,double &m_H, double &m_A, double &m_Hp,
  ...
  if (tan_beta>0) m_A2=m12_2/sb/cb-0.5*v2*(2*lambda[5]+lambda[6]*ctb+lambda[7]*tb);
  else  m_A2=m22_2+0.5*v2*(lambda[3]+lambda[4]-lambda[5]);
...
void THDM::get_param_higgs(double &Lambda1, double &Lambda2, double &Lambda3,
  ...
  Lambda6 = -0.50*s2b*(lambda[1]*cb2-lambda[2]*sb2-lambda345*c2b)+cb*c3b*lambda[6]+sb*s3b*lambda[7];
  Lambda7 = -0.50*s2b*(lambda[1]*sb2-lambda[2]*cb2+lambda345*c2b)+sb*s3b*lambda[6]+cb*c3b*lambda[7];
...
bool THDM::check_perturbativity(double perturbativity_limit) {
  ...
  //cout << "-----> perturbativity  \t" << abs(gmax) << "\t"<< imax<<jmax<<kmax<<lmax <<endl;
...
void THDM::print_param_hybrid() {
  ...
  printf("      m_h: %17.10f\n",mh);
  printf("      m_H: %17.10f\n",mH);
  printf(" cos(b-a): %17.10f\n",cba);
  printf("       Z4: %17.10f\n",Z4);
  printf("       Z5: %17.10f\n",Z5);
  printf("       Z7: %17.10f\n",Z7); 
  printf("tan(beta): %17.10f\n",tb); 
}
...
void THDM::print_param_HHG() {
  ...
  printf(" Lambda_1: %17.10f\n",Lambda1);
  printf(" Lambda_2: %17.10f\n",Lambda2);
  printf(" Lambda_3: %17.10f\n",Lambda3);
  printf(" Lambda_4: %17.10f\n",Lambda4);
  printf(" Lambda_5: %17.10f\n",Lambda5);
  printf(" Lambda_6: %17.10f\n",Lambda6);
  printf("tan(beta): %17.10f\n",tan_beta);
}
...
void THDM::print_param_phys() {
  ...
  printf("      m_A: %17.10f\n",mA);
  printf("     m_H+: %17.10f\n",mHp);
  printf(" sin(b-a): %17.10f\n",sba);
  printf(" lambda_6: %17.10f\n",lambda6);
  printf(" lambda_7: %17.10f\n",lambda7);  
  printf("    m12^2: %17.10f\n",m12_2);
  printf("tan(beta): %17.10f\n",tan_beta);
}
